 ...
 * this.rename_dialog
 * this.client_id
 * this.filecount, how many files in this File Response No Repository File Manager
 * this.maxfiles
 * this.maxbytes
 *
 * FileResponse No Repository FileManager options:
 * =====
...
M.form_fileresponsefilemanager = {};
...
M.form_fileresponsefilemanager.init = function(Y, options) {
    ...
    Y.extend(FileResponseFileManagerHelper, Y.Base, {
        ...
        wait: function(client_id) {
            var container = Y.one('#fileresponsefilemanager-'+client_id);
        ...
        render: function() {
            ...
            var container = Y.one('#fileresponsefilemanager-' + this.client_id);
            var listhtml = '';
    ...
    item = document.getElementById('fileresponsefilemanager-loading-'+options.client_id);
    if (item) {
        item.parentNode.removeChild(item);
    }
    // display filemanager interface
    item = document.getElementById('fileresponsefilemanager-wrapper-'+options.client_id);
    if (item) {
        item.style.display = '';
    }

    new FileResponseFileManagerHelper(options);
};
