...
class qtype_fileresponse_renderer extends qtype_renderer {
    ...
    public function formulation_heading() {
        return get_string('questiontextforcedownload', 'qtype_fileresponse');
    }
    ...
    public function files_input(question_attempt $qa, $numallowed,
        ...
        if ($forcedownload) { /* don't download fix */
            require_once('fileresponsesimplifiedfilemanager.php'); /* don't download fix and check allowed repositories*/
        } else {
        ...
        if ($forcedownload) { /* don't download fix */
            return form_fileresponsesimplifiedfilemanager_render($pickeroptions) . html_writer::empty_tag(
                            'input', array('type' => 'hidden', 'name' => $qa->get_qt_field_name('attachments'),
                        'value' => $pickeroptions->itemid));
        } else {
            return form_fileresponsefilemanager_render($pickeroptions) . html_writer::empty_tag(
                            'input', array('type' => 'hidden', 'name' => $qa->get_qt_field_name('attachments'),
                        'value' => $pickeroptions->itemid));
        }
    }
