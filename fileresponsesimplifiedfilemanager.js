...
/**
 *
 * File Response Simplified File Manager UI
 * =====
 ...
 * this.rename_dialog
 * this.client_id
 * this.filecount, how many files in this File Response Simplified File Manager
 * this.maxfiles
 * this.maxbytes
 *
 * File Response Simplified FileManager options:
 * =====
...
M.form_fileresponsesimplifiedfilemanager = {};
...
M.form_fileresponsesimplifiedfilemanager.init = function(Y, options) {
    ...
    Y.extend(FileResponseSimplifiedFileManagerHelper, Y.Base, {
        ...
        wait: function(client_id) {
            var container = Y.one('#fileresponsesimplifiedfilemanager-'+client_id);
        ...
        setup_buttons: function() {
            ...
            /* here's where the differences to the lib/form/filemanager.js come into place 
        ...
        render: function() {
            ...
            if (path) {
                ...
                for(var p in path) {
                    ...
                    if (count==0) {
                        ...
                        arrow = Y.Node.create('<span> â–¶ </span>');
                    }
                    count++;
            ...
            var container = Y.one('#fileresponsesimplifiedfilemanager-' + this.client_id);
            var listhtml = '';
            ...
            for(var i in list) {
                ...
                html = html.replace('___action___', '<span class="fm-menuicon" id="'+action+'"><img alt="â–¶" src="'+M.util.image_url('i/menu')+'" /></span>');
        ...
        create_filemenu: function(e, data) {
            ...
            var menuitems = [
/*                {text: M.str.moodle.download, url:file.url} don't download fix */
                ];
    ...
    item = document.getElementById('fileresponsesimplifiedfilemanager-loading-'+options.client_id);
    if (item) {
        item.parentNode.removeChild(item);
    }
    // display filemanager interface
    item = document.getElementById('fileresponsesimplifiedfilemanager-wrapper-'+options.client_id);
    if (item) {
        item.style.display = '';
    }

    new FileResponseSimplifiedFileManagerHelper(options);
};
