...
/**
 * File response simplified file manager class.
 *
 * @package    qtype
...
class MoodleQuickForm_fileresponsesimplifiedfilemanager extends HTML_QuickForm_element {
    ...
    function toHtml() {
        ...
        $html = $this->_getTabs();
        $html .= form_fileresponsesimplifiedfilemanager_render($options);
...
class form_filemanaer_x {
    ...
    public function __construct(stdClass $options) {
        ...
        // If file picker plugins aren't allowed make sure that only upload repository is available for students!
 ...
 */
function form_fileresponsesimplifiedfilemanager_render($options) {
    global $CFG, $OUTPUT, $PAGE;

    $fm = new form_filemanaer_x($options); //TODO: this is unnecessary here, the nested options are getting too complex

    static $fileresponsesimplifiedfilemanagertemplateloaded;
    ...
    /* here's where the differences to the lib/form/filemanager.php come into place */
    ...
    $html .= <<<FMHTML
<div class="filemanager-loading mdl-align" id='fileresponsesimplifiedfilemanager-loading-{$client_id}'>
$icon_progress
</div>
<div id="fileresponsesimplifiedfilemanager-wrapper-{$client_id}" style="display:none">
...
<!--        <input type="button" class="fm-btn-download" id="btndwn-{$client_id}" onclick="return false" {$extra} value="{$strdownload}" />  don\'t download fix -->
        <input type="button" class="fm-btn-download" style="color: #808080; visibility:hidden!important;" id="btndwn-{$client_id}" {$extra} value="{$strdownload}" disabled="disabled" /> 
        <br /><span> $maxsize </span>
    </div>
    <div class="filemanager-container" id="fileresponsesimplifiedfilemanager-{$client_id}">
...
FMHTML;
    ...
    $PAGE->requires->js_init_call('M.form_fileresponsesimplifiedfilemanager.init', array($options), true, $module);
